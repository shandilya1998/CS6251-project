...
df = pd.read_csv('emotion_stimulus_dataset_parsed.csv')
...
for i in tqdm(range(len(df))):
    ...
    if emotion == 'happy':
        data.append([df['sentences'][0], 4])
    elif emotion == 'sad':
        data.append([df['sentences'][0], 5])
    elif emotion == 'surprise':
        data.append([df['sentences'][0], 6])
    elif emotion == 'disgust':
        data.append([df['sentences'][0], 2])
    elif emotion == 'anger':
        data.append([df['sentences'][0], 1])
    elif emotion == 'fear':
        data.append([df['sentences'][0], 3])
    elif emotion == 'shame':
        data.append([df['sentences'][0], 7])
    else:
        data.append([df['sentences'][0], 0])

df = pd.read_csv('daily_dialog_dataset_parsed.csv')

for i in tqdm(range(len(df))):
    data.append([df['sentences'][i], df['emotion'][i]])

df = pd.read_csv('parsed_crowdflower_dataset.csv')
